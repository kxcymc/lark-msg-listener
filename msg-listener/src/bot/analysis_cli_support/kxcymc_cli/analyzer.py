...
`kxcymc --json -p <prompt_payload>`：把分析指令和 AnalyzerInput JSON
作为 print 模式的位置参数投喂，兼容当前 kxcymc CLI 行为。
"""
...
- 如输入中包含http(s)链接，必须在输出 prompt 中原样保留这些链接，不要省略、改写或只总结链接内容。
"""
...
_RATE_LIMIT_PATTERNS = [
    ...
    re.compile(r"服务繁忙"),
    re.compile(r"请稍后", re.IGNORECASE),
    re.compile(r"quota", re.IGNORECASE),
]


@dataclass
class kxcymcCliConfig:
    command: str = "kxcymc"
    args: list[str] = field(default_factory=lambda: ["--json", "-p"])
    prompt_via: str = "argv"
...
def _extract_target_payload(obj: object, *, depth: int = 0) -> dict | None:
    ...
    if isinstance(obj, dict):
        if obj.get("category") in VALID_CATEGORIES:
            return obj
        priority_keys = ("result", "data", "content", "message", "messages", "agent_states")
...
class kxcymcCliAnalyzer:
    """通过 kxcymc CLI 子进程分析 record。"""

    name = "kxcymc"

    def __init__(self, config: kxcymcCliConfig) -> None:
        self.config = config
    ...
    async def analyze(self, payload: AnalyzerInput) -> AnalyzerResult:
        ...
        stdin_bytes = self._build_stdin(payload)

        logger.debug("kxcymc exec: %s", " ".join(argv))
        try:
            ...
            raise AnalyzerError(
                f"未找到 kxcymc 可执行文件：{self.config.command}",
                retriable=False,
            ) from exc
        ...
        try:
            ...
            await proc.wait()
            raise AnalyzerError(
                f"kxcymc 子进程超时 (> {self.config.timeout_seconds:.0f}s)",
                retriable=True,
            ) from exc
        ...
        if rc != 0:
            if _looks_like_rate_limit(stderr) or _looks_like_rate_limit(stdout):
                raise AnalyzerRateLimited(
                    f"kxcymc 限流/服务繁忙 (rc={rc}): {stderr.strip()[:200]}",
                    raw_stdout=stdout,
                    raw_stderr=stderr,
                )
            raise AnalyzerError(
                f"kxcymc 子进程失败 (rc={rc}): {stderr.strip()[:400]}",
                retriable=True,
                raw_stdout=stdout,
                raw_stderr=stderr,
            )
